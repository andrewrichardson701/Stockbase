...
function modalLoadAddChildren(container_id, is_item) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/_ajax-nearbystock?item_id="+container_id+"&is_item="+is_item, true);
    xhr.onload = function() {
        if (xhr.status === 200) {
            // Parse the response and populate the shelf select box
...
function modalLoadUnlinkContainer(containerID, itemID, is_item) {
    ...
    var containerName = document.getElementById("container-"+containerID+"-name").innerHTML;
